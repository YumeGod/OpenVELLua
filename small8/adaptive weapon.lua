...
local check = ui.new_checkbox("rage", "other", "delay shot on hotkey")
local hotkey = ui.new_hotkey("rage", "other", "dealy shot on hotkey", true)
local edge = ui.reference("rage", "other", "Delay shot")

local state = false
local function on_paint()
	state = ui.get(hotkey)
	if state then
		renderer.indicator(207, 206, 204, 255, "Delay")
	end
end

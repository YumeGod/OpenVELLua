...
local function render_circle(x, y, r, g, b, a, start_degrees)
	renderer.circle_outline(x, y, 0, 0, 0, 155, 10, 0, 1, 6)
	renderer.circle_outline(x, y, r, g, b, a, 9, start_degrees > 0 and 90 or -90, 0.5, 4)
end
...
local function get_color(r)
	return 255 - 155 * r, 240 * r, 0
end
...
local function draw_direction(c)
	...
	local pose = entity.get_prop(local_player, "m_flPoseParameter", 11) * 120 - 60
	local head_pos = vector(entity.hitbox_position(local_player, "head_0"))
	local pelvis_pos = vector(entity.hitbox_position(local_player, "pelvis"))
	local normalized_yaw = normalize_yaw(CalcAngle(head_pos.x, head_pos.y, pelvis_pos.x, pelvis_pos.y) - vector(client.camera_angles()).y + 120)

	local rr, gg, bb = get_color(math.abs(pose) / 60)

	render_circle(center_x, center_y + 4, rr, gg, bb, 255, pose / 60)
	renderer.circle_outline(center_x, center_y + 4, rr, gg, bb, 255, 18, normalized_yaw * -1 - 15, 0.2, 4)
	...
	if ui.get(legit_aa_on_key) then
		...
		if ui.get(change_aa_on_key) then
			...
			-- if ui.get(enable_custom_aa) then
				...
				-- ui.set(air.yaw_slider, -21)
				-- ui.set(slow_walk.yaw_slider, -21)
			-- else
				ui.set(yaw_add, -90)
			--end
				do_it_once = false

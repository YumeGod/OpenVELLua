...
local script = {
	...
	reference = { },
	interface = {
		ui.new_checkbox('Rage', 'Other', 'Double tap indicator'),
	}
}
...
local initialization = function()
	...
	local hitchance = script:ui_register('hitchance', { 'RAGE', 'Other', 'Double tap hit chance' })
	local mindamage = script:ui_register('mindamage', { 'RAGE', 'Aimbot', 'Minimum damage' })
    ...
    -- Plugin references
	local master_switch = script:ui_register('master_switch', { 'RAGE', 'Other', 'Double tap indicator' })
    ...
    local g_aimbot_listener = function(e)
		...
		mindamage:set_cache(1, run_qm, 1)
    ...
    local g_command_controller = function(e)
		...
		mindamage:set_cache(1, caimbot.shift_time > 0 or fired_this_tick, 0)
	end
    ...
    local g_paint_handler = function()
		...
		note.get(function(id)
            ...
            local text = string.format('DT [%s] | tickbase(v): %s | DT: %s', script.version, cmove.last_charge, dt_ready)

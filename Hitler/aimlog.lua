...
local on_hurt_enable = ui.new_checkbox("misc", "miscellaneous", "Log damage received")
    ...
    if attacker_id ~= entity.get_local_player() then
        if ui.get(on_hurt_enable) then
            ...
            if target_id ~= entity.get_local_player() then
                ...
                local attacker_name = entity.get_player_name(attacker_id)
            local rhp = ""
            client.log("Take ",e.dmg_health," damage in ",group," from ",attacker_name)
            end
        end
    else

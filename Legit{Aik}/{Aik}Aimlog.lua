...
local aimbotlog_enable = ui.new_checkbox("Rage", "Other", "中文控制台")
local on_fire_enable = ui.new_checkbox("Rage", "Other", "Fire log color")
local on_fire_colour = ui.new_color_picker("Rage", "Other", "Fire color", 147, 112, 219, 255)
local on_miss_enable = ui.new_checkbox("Rage", "Other", "Miss color")
local on_miss_colour = ui.new_color_picker("Rage", "Other", "Miss color", 255, 253, 166, 255)
local on_damage_enable = ui.new_checkbox("Rage", "Other", "Damage color")
local on_damage_colour = ui.new_color_picker("Rage", "Other", "Damage color", 100, 149, 237, 255)
...
local function on_aim_fire(e)
    if ui_get(aimbotlog_enable) and ui_get(on_fire_enable) and e ~= nil then
        ...
        local hitgroup_names = { "BAIM", "头", "胸", "胃", "左胳膊", "右胳膊", "左腿", "右腿", "脖子", "?"}
        ...
        "[VEL.GAMESENSE] 击中了 ", string.lower(target_name),
        ", 击中部位: ", group,
        "  伤害: ", e.damage,
...
local function on_player_hurt(e)
    ...
    local message = "[VEL.GAMESENSE] 击中 " .. string.lower(target_name) .. ", 命中部位: " .. group .. "  伤害: " .. e.dmg_health .. "  剩余血量: " .. rem_health
    if rem_health <= 0 then
        message = message .. " (emo了)"
    end
...
local function on_aim_miss(e)
    ...
    if e.reason == "?" then
        ...
        client_color_log(r, g, b,
        "[VEL.GAMESENSE] 正常操作了 ", string.lower(target_name),
        ", 击中部位: ", group,
        "  操作原因: ", reason)
    end
end
